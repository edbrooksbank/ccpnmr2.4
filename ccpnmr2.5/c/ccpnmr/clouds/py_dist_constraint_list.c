...
{
    int i;
    Dist_constraint_list dist_constraint_list = new_dist_constraint_list(0, NULL, NULL, NULL, NULL);
...
static PyObject *add(PyObject *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "iiff", &atom0, &atom1, &dist_lower, &dist_upper))
        RETURN_OBJ_ERROR("must have four arguments: atom0, atom1, dist_lower, dist_upper");

    py_dist_constraint = new_py_dist_constraint(atom0, atom1,
...
PY_MOD_INIT_FUNC initDistConstraintList(void)
{
    PyObject *m, *d;
    ...
    m = Py_InitModule("DistConstraintList", Dist_constraint_list_type_methods);

    /* create exception object and add to module */
    ErrorObject = PyErr_NewException("DistConstraintList.error", NULL, NULL);
    Py_INCREF(ErrorObject);
    PyModule_AddObject(m, "error", ErrorObject);
    
    /* check for errors */
    if (PyErr_Occurred())
        Py_FatalError("can't initialize module DistConstraintList");
}
